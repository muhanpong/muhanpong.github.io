...
### 9. WASM Stability & Lifecycle Fixes
- **Problem**: Concurrent access to the WASM encoder and oversized audio tails were causing panics and "borrowed" errors.
- **Solution**: 
  - Implemented an **`isBusy` lock** in [worker.js](file:///c:/Users/povian/git/muhanpong.github.io/worker.js) to serialize all WASM interactions.
  - Ensured **`encoder.free()`** is called after the `FINISH` event to properly release Rust memory and object ownership.
  - Added an **8KB clamp** to `remainingMp3` in [encoder.html](file:///c:/Users/povian/git/muhanpong.github.io/encoder.html) to prevent out-of-bounds slice errors in the MV2 writer.
