...
export class Mv2Encoder {
    __destroy_into_raw() {
        ...
        this.__wbg_ptr = 0;
        Mv2EncoderFinalization.unregister(this);
        return ptr;
    }
    free() {
        const ptr = this.__destroy_into_raw();
        wasm.__wbg_mv2encoder_free(ptr, 0);
    }
    /**
    ...
    constructor(config_json) {
        ...
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.mv2encoder_new(ptr0, len0);
        if (ret[2]) {
...
function __wbg_get_imports(memory) {
    ...
    return {
        __proto__: null,
        "./mv2_wasm_bg.js": import0,
    };
}

const Mv2EncoderFinalization = (typeof FinalizationRegistry === 'undefined')
    ? { register: () => {}, unregister: () => {} }
    : new FinalizationRegistry(ptr => wasm.__wbg_mv2encoder_free(ptr >>> 0, 1));
...
async function __wbg_init(module_or_path, maybe_memory) {
    if (wasm !== undefined) return wasm;
    ...
    if (module_or_path !== undefined) {
        if (Object.getPrototypeOf(module_or_path) === Object.prototype) {
            ({module_or_path, memory: maybe_memory, thread_stack_size} = module_or_path)
        } else {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('mv2_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports(maybe_memory);
    if (maybe_memory) { imports.env = { memory: maybe_memory }; }
